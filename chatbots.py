            ...
            with st.expander("View Top Chunks", expanded=False):
                ...
                if len(top_references) == len(sources) == len(metadata):
                    for chunk_num in range(min(5,len(top_references))):
                        ...
                        if chunk_page: st.markdown(f"``` Source: {sources[chunk_num]} || Page: {chunk_page}```")
                        else: st.markdown(f"``` Source: {sources[chunk_num]} ```")
                        ...
                        st.markdown("___")
                elif full_response.get("testing"):
                    # st.markdown(full_response)
                    top_sections = top_references.get("top_sections",[])
                    top_chunks = top_references.get("top_chunks",[])
                    # "```markdown\n"
                    #         f"{sec.get('section_text', 'No section text available.')}\n"
                    #         "```"
                    for sec_num, sec in enumerate(top_sections):
                        st.markdown(f"#### Section {sec_num+1}: \n**Page:** {sec['page_idx']} || **Title:** {sec['title']}\n")
                    for chk_num, chk in enumerate(top_chunks):
                        st.markdown(f"##### Chunk {chk_num+1}: \n**Text:** {chk['text']} \n\n**Score:** {chk['score']} || **Chunk Id:** {chk['chunk_id']}\n")
                else:
