...
# -------------------------------------------------------
# 2) Session State for Chat History and Saved Prompt
# -------------------------------------------------------
if "hybrid_chatHistory" not in st.session_state:
    st.session_state["hybrid_chatHistory"] = []
if "saved_prompt" not in st.session_state:
    ...
    st.session_state["saved_prompt"] = (
        ...
        "If the user asked the question in English, respond strictly in English. If in Hinglish, respond in Hinglish.\n"
        "Do not exceed 50 words in your initial response. If the user asks for more details, expand your response up to 150 words.\n"
        "While replying in Hinglish, ensure context-aware translation spoken as a female.\n"
        "Answer strictly based on the provided context. Do not incorporate outside information or hallucinate.\n"
        "For too general questions, ask a clarifying follow-up only once.\n"
        "If a greeting is detected, welcome the user and introduce yourself.\n"
        "If the user replies 'yes', generate a standalone question that precisely asks for the additional information present in the source documents."
    )

# -------------------------------------------------------
# 3) Text Area for Prompt Input with Save Button
# -------------------------------------------------------
st.markdown("### Customize Bot Prompt")
user_prompt = st.text_area("Enter your custom prompt (multi-line):", value=st.session_state["saved_prompt"], height=250)
if st.button("Save Prompt"):
    st.session_state["saved_prompt"] = user_prompt
    st.success("Prompt saved!")

st.markdown("---")
st.write(f"**Current API Base URL:** {st.session_state.get('base_url', 'http://127.0.0.1:8000')}")

# -------------------------------------------------------
# 4) User Input for the Question
# -------------------------------------------------------
user_question = st.text_input("Enter your question:", "")
...
# -------------------------------------------------------
# 5) API Base URL (can be changed via a text input)
# -------------------------------------------------------
if "base_url" not in st.session_state:
    st.session_state["base_url"] = "http://127.0.0.1:8000"
with st.form(key='base_url_form'):
    base_url_input = st.text_input("Enter API base URL (e.g., http://127.0.0.1:8000 or https://<ngrok_link>):", 
                                   value=st.session_state["base_url"])
    submit_button = st.form_submit_button(label='Save')
    if submit_button:
        new_url = base_url_input.rstrip("/")
        st.session_state["base_url"] = new_url
        st.success(f"API base URL updated to: {new_url}")

# -------------------------------------------------------
# 6) Hybrid Bot Endpoint Setup
# -------------------------------------------------------
base_url = st.session_state["base_url"]
url_hBot = f"{base_url}/chatbotazure/retrieve-response-new-bot/"

# -------------------------------------------------------
# 7) Button to Ask the Hybrid Bot
# -------------------------------------------------------
if st.button("Ask Hybrid Bot"):
    if user_question.strip() == "":
        st.warning("Please enter a question before asking the bot.")
        ...
        with st.spinner("Waiting for the Hybrid Bot to respond..."):
            try:
...
def display_chat(chat_history):
    ...
    for idx, msg in enumerate(chat_history):
        if msg["role"] == "assistant":
            st.markdown(
                f"""
                <div style='text-align: left; background-color: #F0F0F0; padding: 15px; margin: 10px 0; border-radius: 8px;'>
                    <b>Assistant:</b> {msg['content']}
                </div>
                """, unsafe_allow_html=True)
        elif msg["role"] == "user":
            st.markdown(
                f"""
                <div style='text-align: right; background-color: #D0F0FF; padding: 15px; margin: 10px 0; border-radius: 8px;'>
                    <b>You:</b> {msg['content']}
                </div>
                """, unsafe_allow_html=True)
